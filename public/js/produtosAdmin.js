...
document.addEventListener("DOMContentLoaded", () => {
  ...
  loadeEspeciesOptions(especieSelect);
  initSidebar();
...
function wireProductEditModal() {
  ...
  const descricaoInput = document.getElementById('product-descricao');
  const especieInput = document.getElementById('product-especie');
  ...
  function openModal(isNew = false, data = {}) {
    ...
    if (isNew) {
      ...
      especieInput.value = data.especie
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    const payload = {
      ...
      especie: especieInput.value || null
    };
...
async function loadCategoriasOptions(selectEl, selectedKey = '') {
  ...
  try {
    const categorias = await fetchJSON(`${API_BASE}/produtos/categorias`);
    const opts = [
...
async function loadeEspeciesOptions(selectEl, selectedKey = '') {
  if (!selectEl) return;
  // placeholder enquanto carrega
  selectEl.innerHTML = `<option value="">Carregando…</option>`;

  try {
    const categorias = await fetchJSON(`${API_BASE}/produtos/especies`);
    const opts = [
      `<option value="" disabled ${selectedKey ? '' : 'selected'}>Selecione…</option>`,
      ...categorias.map(c => `<option value="${c.key}">${c.value}</option>`)
    ];
